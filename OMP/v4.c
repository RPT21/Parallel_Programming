#include <math.h>
#include <stdio.h>
#include <stdlib.h>

void VectF1 ( double *IN, double *OUT, int n)
{
  for ( int i=0; i<n; i++ ) {
    long int T = IN[i];
    OUT[i] = (double) (T % 4) + 0.5 + (IN[i]-trunc(IN[i]));
  }
}

void VectF2 ( double *IN, double *OUT, double v, int n)
{
  for ( int i=0; i<n; i++ )
    OUT[i] = v / ( 1.0 + fabs(IN[i]));
}

void VectScan ( double *IN, double *OUT, int n)
{
  double sum = 0.0; 
  for ( int i=0; i<n; i++ )
  {
    sum += IN[i];
    OUT[i] = sum; // Inclusive: include current element
  }
}

void VectAverage ( double *IN, double *OUT, int n)
{
  for ( int i=1; i<n-1; i++ ) {
    OUT[i] = (2.0*IN[i]+IN[i-1]+IN[i+1])/4.0;
  }
}

double VectSum (double *V, int n)
{
  double sum = 0;
  for ( int i=0; i< n; i++ )
    sum = sum + V[i];
  return sum;
}

int main(int argc, char** argv)
{
  int i, N=20000, REP=250000;

  // Get program arguments at runtime
  if (argc>1) {  N   = atoi(argv[1]); }
  if (argc>2) {  REP = atoi(argv[2]); }

  // Allocate memory space for arrays
  double *A = malloc ( N*sizeof(double) );
  double *B = malloc ( N*sizeof(double) );
  double *C = malloc ( N*sizeof(double) );
  double *D = malloc ( N*sizeof(double) );

  //  set initial values
  srand48(0);
  for (i=0; i < N; i++)
    A[i] = drand48()-0.5f; // values between -0.5 and 0.5

  printf("Inputs: N= %d, Rep= %d\n", N, REP);

  double  v = 10.0;
  for( i=0; i<REP; i++ ){ 
    #pragma omp parallel num_threads(6) shared (A,B,C,D,v)
        {
        #pragma omp sections
        {
            #pragma omp section
            {
                VectF1(A, B, N/6);
                VectF2(B, C, v, N/6);
            }
            #pragma omp section
            {
                VectF1(A + N/6, B + N/6, N/6);
                VectF2(B + N/6, C + N/6, v, N/6);
            }
            #pragma omp section
            {
                VectF1(A + N/3, B + N/3, N/6);
                VectF2(B+N/3, C+N/3, v, N/6);
            }
            #pragma omp section
            {
                VectF1(A + N/2, B + N/2, N/6);
                VectF2(B+N/2, C+N/2, v, N/6);
            }
            #pragma omp section
            {
                VectF1(A + 2*N/3, B + 2*N/3, N/6);
                VectF2(B+2*N/3, C+2*N/3, v, N/6);
            }
            #pragma omp section 
            {
                VectF1(A + 5*N/6, B + 5*N/6, N - 5*N/6);
                VectF2(B+5*N/6, C+5*N/6, v, N - 5*N/6); /* the last could be unbalanced */
            }    
            
        }

        #pragma omp sections  
            {
            #pragma omp section
                {
                VectScan(C, A, N);
                }
            #pragma omp section
                {
                VectAverage(B, D, N);
                v = VectSum(D, N);
                }
            }
        }
        
    }
     
  printf("Outputs: v= %0.12e, A[%d]= %0.12e\n", v, N-1, A[N-1]);

  // Free memory space for arrays
  free(A); free(B); free(C); free(D);
}