...
int main(int argc, char** argv){
  ...
  while(error > tol && iter < iter_max){
    if(me > 0){
      MPI_Send(A[first_row], m, MPI_FLOAT, me - 1, 66, MPI_COMM_WORLD);
      MPI_Recv(A[last_row], m, MPI_FLOAT, me - 1, 66, MPI_COMM_WORLD, &s);
    }
    if(me < nproc - 1){
      MPI_Send(A[last_row], m, MPI_FLOAT, me + 1, 66, MPI_COMM_WORLD);
      MPI_Recv(A[first_row], m, MPI_FLOAT, me + 1, 66, MPI_COMM_WORLD, &s);
    }
